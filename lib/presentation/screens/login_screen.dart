...
class _LoginScreenState extends State<LoginScreen> {
  ...
  bool _isAdmin = false; // Chỉ sử dụng khi đăng ký
  
  final _auth = auth.FirebaseAuth.instance;
  final _getUserById = GetIt.instance<GetUserById>();
  final _createAdminUser = GetIt.instance<CreateAdminUser>();
  ...
  Future<void> _fixAdminRole(String uid, String email) async {
    try {
      // Thêm danh sách email admin mặc định - CHỈ những email này mới có quyền admin
      final adminEmails = [
        'admin@example.com',
        'admin@admin.com', 
        // Thêm các email admin cụ thể vào đây
      ];
      
      print('Kiểm tra quyền admin cho email: $email');
      
      if (adminEmails.contains(email.toLowerCase())) {
        print('Email $email nằm trong danh sách admin. Tiến hành cập nhật quyền...');
        ...
        print('Email $email KHÔNG nằm trong danh sách admin.');
        ...
        if (user != null && user.role == UserRole.admin) {
          print('Phát hiện tài khoản không phải admin nhưng có quyền admin. Sửa lại thành user...');
          await userRepo.updateUserRole(uid, UserRole.user);
          print('Đã cập nhật lại quyền thành user.');
        }
      }
    } catch (e) {
    ...
    try {
      if (_isLogin) {
        ...
        if (userCredential.user != null) {
          print('Đăng nhập thành công. UserID: ${userCredential.user!.uid}');
          
          // Thử sửa quyền admin
          await _fixAdminRole(userCredential.user!.uid, email);
          
          // Kiểm tra quyền của người dùng
          print('Đang kiểm tra quyền user...');
          final user = await _getUserById(userCredential.user!.uid);
          
          // Debug
          print('Kết quả getUserById: ${user?.toMap()}');
          ...
          if (user != null && user.role == UserRole.admin) {
            print('Người dùng là ADMIN. Chuyển đến AdminHomeScreen');
            // Nếu là admin, chuyển đến màn hình admin
            Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (context) => const AdminHomeScreen()),
            );
          } else {
            print('Người dùng là USER thường. Chuyển đến HomePage');
        ...
        if (_isAdmin) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Đã tạo tài khoản admin thành công!'),
          ...
          child: Column(
            ...
            children: [
              ...
              if (!_isLogin) ...[
                const SizedBox(height: 16),
                CheckboxListTile(
                  title: const Text('Đăng ký tài khoản admin'),
                  value: _isAdmin,
                  onChanged: (value) {
                    setState(() {
              ...
              TextButton(
                onPressed: () {
                  setState(() {
                    _isLogin = !_isLogin;
                    _isAdmin = false; // Reset admin option when switching
                  });
                },
                child: Text(
                  _isLogin
